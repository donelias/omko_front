 ...
 *
 * Configuración centralizada de optimización de imágenes para OMKO Web,
 * alineada con el pipeline del backend (FileService comprime a 80% y los
 * jobs de Spatie optimizan en background). Aquí se definen tamaños
 * responsivos, calidades por formato y notas de configuración de servidor.
 */
...
const IMAGE_OPTIMIZATION_CONFIG = {
  // Formatos soportados en orden de preferencia (JPEG es el fallback implícito)
  ...
  // Tamaños predefinidos para next/image (atributo sizes)
  sizes: {
...
/**
 * Configuración de compresión para imágenes servidas por Next.js Image
 */
...
/**
 * Configuración Next.js (referencia: mover al key "images" de next.config.js).
 * En next.config.js los remotePatterns se construyen desde
 * NEXT_PUBLIC_IMAGE_HOSTS (coma-separada), ver ese archivo.
 */
const NEXT_CONFIG_IMAGES = {
  images: {
    // En local/dev se puede desactivar con NEXT_IMAGE_UNOPTIMIZED=true;
    // en producción queda false para que Next sirva WebP/AVIF.
    unoptimized: false,

    // Formatos permitidos (JPEG es el fallback implícito)
    formats: ['image/avif', 'image/webp'],

    // Dispositivos para srcset
    deviceSizes: [640, 750, 828, 1080, 1200, 1920, 2048, 3840],

    // Caché en el navegador (segundos) — 1 año
    minimumCacheTTL: 31536000,

    // Soportar SVG del admin (iconos de categoría/ciudad)
 ...
 * Aplicada en el .htaccess del directorio raíz (ver ese archivo).
 */
