...
const PropertyHorizontalCard = ({
  ...
  const renderDualPrices = () => {
    ...
    // CONDICIÓN FLEXIBLE: Acepta tanto DOP como RD$
  ...
  return (
    <div aria-label={`${property?.translated_title || property?.title} in ${property?.state}`} className="group">
      <article className="cardBg lg:h-[250px] hover:shadow-lg transition-all duration-500 cardBorder mx-auto grid grid-cols-1 overflow-hidden rounded-2xl lg:grid-cols-12">
        <figure className="relative col-span-5 group-hover:cursor-pointer">
          ...
          <button className="p-2 rounded-lg absolute top-2 right-2 bg-black/50 text-white" aria-label="Add to favourites" onClick={handleFavourite}>
        ...
        <div className="md:col-span-7 w-full h-fit flex flex-col gap-2 cardBg group-hover:cursor-pointer" onClick={handlePropertyClick}>
          <header className="mx-2 p-2 flex-grow h-fit">
            <div className="flex justify-between items-center">
              ...
              <span className={`${property?.property_type === "sell" ? "primarySellBg primarySellText" : "primaryRentBg primaryRentText"} rounded-[100px] px-3 py-1 text-sm font-bold`} itemProp="offeredBy">
          ...
          <div>
            ...
            <div className="flex items-center gap-6 px-4 py-3" itemProp="additionalProperty">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger className="!w-fit">
                          <div className="flex items-center gap-2 overflow-hidden text-ellipsis" aria-label={parameter?.translated_name || parameter?.name}>
                            ...
                            <span className="font-medium leadColor" itemProp="numberOfRooms">
          ...
          <footer className="mx-2 flex items-center justify-between p-2 min-h-[56px]">
            {renderDualPrices()}
            {!property?.is_premium && <div className="h-11" />}
            {property?.is_premium && (
              <span className={`rounded-[100px] primaryRentBg p-2 font-semibold text-base flex items-center gap-2`}>
