...
const FeaturedPropertyHorizontalCard = ({ property, removeCard = null }) => {
    ...
    const renderDualPrices = () => {
        ...
        // Limpieza total de strings para evitar fallos por espacios invisibles
        ...
        // CONDICIÓN FLEXIBLE: Ahora evalúa DOP y RD$ de forma segura
        ...
        return (
            <div key={rate} className="flex flex-col leading-tight">
                <p className="text-sm text-gray-500 mb-0.5">{t('price')}</p>
                <span className="text-xl font-bold text-white">
                    {mainPriceString}
                    {property?.property_type === "rent" && property?.rentduration && (
                        <span className="text-xs font-normal text-gray-400 ml-1">
                            / {t(property.rentduration.toLowerCase())}
                        </span>
                    )}
                </span>
                <span className="text-[11px] font-medium text-gray-400 mt-0.5 tracking-wide block">
    ...
    const handleLike = async (e) => {
        ...
        if (!isUserLoggedIn) {
            ...
            }).then((result) => { if (result.isConfirmed) setShowLoginModal(true); });
            return;
        }

        try {
            setIsLoading(true); setIsLiked(true); setIsDisliked(false);
            const res = await addFavouritePropertyApi({ property_id: property?.id, type: "1" });
            if (res.error === false) toast.success(t(res?.message));
            else { setIsLiked(false); toast.error(res.message); }
        } catch (error) {
            setIsLiked(false);
    ...
    const handleDislike = async (e) => {
        ...
        try {
            setIsLoading(true); setIsLiked(false); setIsDisliked(true);
            const res = await addFavouritePropertyApi({ property_id: property?.id, type: "0" });
            if (res.error === false) {
                toast.success(t(res?.message));
                if (removeCard) removeCard(property?.id);
            } else { setIsLiked(true); setIsDisliked(false); toast.error(t(res?.message)); }
        } catch (error) {
            setIsLiked(true); setIsDisliked(false);
        ...
        <div className="group bg-[#181818] rounded-2xl overflow-hidden border border-transparent transition-all duration-300 flex flex-col sm:flex-row p-3 sm:p-4 md:p-5 lg:p-6 gap-4 sm:gap-5 md:gap-6">
            <div className="relative w-full sm:w-[300px] md:w-[360px] lg:w-[390px] shrink-0" >
                <CustomLink onClick={handlePropertyClick} href={`${isUserProperty ? `/my-property/${property?.slug_id}` : `/property-details/${property?.slug_id}`}`} aria-label={property?.translated_title || property?.title}>
                    <ImageWithPlaceholder
                        ...
                        className="w-full h-full object-cover rounded-2xl aspect-[390/310]"
                        loading='lazy'
                    />
                </CustomLink>
                        ...
                        <ReactSVG
                            ...
                            beforeInjection={(svg) => {
                                ...
                                svg.querySelectorAll("path").forEach((path) => {
                                    path.setAttribute("style", `fill: var(--featured-animated-color);`);
                                });
                            }}
                ...
                <button onClick={isLiked ? handleDislike : handleLike} disabled={isLoading} aria-label={isLiked ? "Remove from favorites" : "Add to favorites"} className={`absolute top-3 right-3 bg-[#00000080] hover:bg-[#00000090] text-white p-1.5 rounded-md transition-colors duration-200 backdrop-blur-sm ${isLoading ? 'opacity-50 cursor-not-allowed' : 'hover:scale-110'}`}>
                    {isLiked ? <FaHeart className="text-lg text-white" /> : <FiHeart className="text-lg text-white" />}
                </button>
            </div>
            ...
            <div className="p-2 sm:p-3 md:p-4 lg:p-5 flex flex-col flex-grow">
                ...
                <h3 className="text-lg font-semibold text-white mb-1" title={property?.title}>
                    <CustomLink onClick={handlePropertyClick} href={`${isUserProperty ? `/my-property/${property?.slug_id}` : `/property-details/${property?.slug_id}`}`} className="hover:text-gray-300 transition-colors line-clamp-1">
                                ...
                                {validParameters.map((parameter, index) => (
                                    <Tooltip key={parameter.id || index}>
                                        <TooltipTrigger asChild>
                                            <div className="flex items-center justify-start gap-1 px-2 cursor-default">
                                                <div className="bg-white/25 p-1.5 flex items-center justify-center rounded-full">
                                                    {parameter?.image ? (
                                                        <ReactSVG
                                                            src={parameter.image}
                                                            beforeInjection={(svg) => {
                                                                svg.setAttribute("style", `height: 17px; width: 17px;`);
                                                                svg.querySelectorAll("path").forEach((path) => {
                                                                    path.setAttribute("style", `fill: var(--featured-animated-color);`);
                                                                });
                                                            }}
                                                ...
                                                <span className="truncate">{getDisplayValueForOption(parameter)}</span>
                                            </div>
                                        </TooltipTrigger>
                ...
                <div className="flex items-center justify-between mt-auto pt-3 sm:pt-4">
                    ...
                    >
                        <ArrowRight size={18} className={`flex-shrink-0 ${isRtl ? "rotate-180" : ""}`} />
