...
import { setLocationAction, clearLocationAction } from "@/redux/slices/locationSlice";
...
const LocationSearchWithRadius = ({ isOpen, onClose }) => {
    ...
    const maxRadius = systemSettings?.max_radius ?? 50;
    ...
    const handlePlaceSelect = (placeData, placeDetails) => {
        if (placeData) {
            ...
            setSelectedLocation({ lat, lng, address: placeData.formatted_address, city, state, country, radius });
    ...
    // Clear handler
    const handleClear = () => {
        // Dispatch clearLocationAction to reset Redux location state to empty
        dispatch(clearLocationAction());

        // Reset local component state
        const defaultLat = Number(webSettings?.latitude) || defaultCenter.lat;
        const defaultLng = Number(webSettings?.longitude) || defaultCenter.lng;
        setLocation({ lat: defaultLat, lng: defaultLng, radius: minRadius });
        setSelectedLocation({
            lat: defaultLat,
            lng: defaultLng,
            address: "",
    ...
    useEffect(() => {
        if (isOpen) {
            // Increment key every time the dialog opens so LeafletMapView fully
            // remounts on a fresh DOM node, preventing the _leaflet_events crash.
        ...
        <Dialog open={isOpen} onOpenChange={(open) => {
            ...
            <DialogContent
                ...
                <div className="p-4">
                    ...
                    <div className="flex justify-center primaryBackgroundBg items-center border rounded-lg p-1 mb-4 relative z-[9999]">
