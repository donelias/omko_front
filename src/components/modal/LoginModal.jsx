...
const LoginModal = ({ showLogin, setShowLogin }) => {
  ...
  useCountdownTimer(isCounting, timeLeft, setTimeLeft, setIsCounting);
  useCountdownTimer(isEmailCounting, emailTimeLeft, setEmailTimeLeft, setIsEmailCounting, false);
  ...
  const handleForgotPasswordSubmit = async (email) => {
    ...
    try {
      ...
      if (response.error !== true) {
        ...
        toast.error(t(response?.message) || t("failedToSendOTP"));
        setShowLoader(false);
      }
    } catch (error) {
  ...
  const handleEmailForgotPasswordUpdate = async (e) => {
    ...
    try {
      ...
      if (!updateResponse.error) {
        // Step 2: Auto-login after password reset
        const res = await userSignup({
          type: "3",
          email: forgotPasswordEmail,
          password: phonePassword,
      ...
      setShowLoader(false);
      console.error("Email forgot password update error:", error);
      toast.error(t(error?.message) || t("somethingWentWrong"));
    }
  };
