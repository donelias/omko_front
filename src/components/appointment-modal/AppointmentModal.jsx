...
const AppointmentScheduleModal = ({
        ...
        try {
            ...
            if (response?.data) {
                ...
                trackEvent(
                    ...
                    { item_id: selectedProperty?.id, meeting_type: selectedMeetingType },
                    { clarity: true }
                );
                onClose && onClose();
            } else {
    ...
    return isMobile ? (
        ...
        <Dialog open={isOpen} onOpenChange={handleOpenChange}>
            <DialogContent
                className="max-w-[310px] md:max-w-7xl rounded-2xl w-full p-0 gap-0 max-h-[500px] md:max-h-[80vh] 2xl:max-h-[900px] !border-0 overflow-auto outline-none custom-scrollbar [&>button]:hidden"
                onInteractOutside={(e) => e.preventDefault()}
            >
                {renderContent()}
            </DialogContent>
