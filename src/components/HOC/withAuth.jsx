...
const withAuth = (WrappedComponent) => {
    const Wrapper = (props) => {
        ...
        // Private routes keep the client-side auth guard. Public routes render
        // immediately (SSR included) so search engines see the real content
        // instead of a Loader shell.
        const isPrivateRoute = privateRoutes.some((route) =>
            currentPath === route || currentPath.startsWith(`${route}/`)
        );

        const [isAuthorized, setIsAuthorized] = useState(!isPrivateRoute);
        const [authChecked, setAuthChecked] = useState(!isPrivateRoute);
