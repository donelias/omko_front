...
import { isRTL, generateBase64FilterUrl, truncate } from "@/utils/helperFunction";
    ...
    <div className="relative w-full h-full pb-6 sm:pb-0">
      {hasSlides ? (
                            ...
                            <div className="flex items-center justify-between w-full">
                              <div className="text-left">
                                <div className="text-lg md:text-xl font-bold primaryColor">
                                  {getDualPriceDisplay(image?.property?.price, image?.property?.currency).mainPriceString} <span className="text-sm font-normal">{image?.property?.property_type === "rent" && image?.property?.rentduration ? "/ " + image?.property?.rentduration : ""}</span>
                                </div>
                                <div className="text-xs md:text-sm text-gray-500 font-medium">
                                  {getDualPriceDisplay(image?.property?.price, image?.property?.currency).convertedPriceString}
                                </div>
                              </div>
                              <CustomLink
        ...
        <div className="w-full h-full bg-gradient-to-b from-slate-100 to-slate-200">
          <div className="w-full aspect-[1920/1080] lg:aspect-[1920/700] xl:aspect-[auto/700]" />
        </div>
      )}
