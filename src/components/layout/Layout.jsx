...
const Layout = ({ children }) => {
  ...
  const fetchWebSettings = useCallback(async () => {
    try {
      ...
      if (data?.web_favicon) {
        const faviconUrl = data.web_favicon.startsWith('http') 
          ? data.web_favicon 
          : `${process.env.NEXT_PUBLIC_BACKEND_URL || process.env.NEXT_PUBLIC_API_URL || 'https://adminrealestate.omko.do/public'}/storage/${data.web_favicon}`;
