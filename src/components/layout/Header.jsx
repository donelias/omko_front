...
const Header = () => {
  ...
  // Hydration-safe gate: durante SSR y el primer render de hidratación, los
  // datos de redux-persist (lang, usuario, ubicación, web settings) NO están
  // disponibles en el servidor, así que los renderizamos igual al servidor y
  // los aplicamos solo tras mount (mismo patrón que Layout).
                  ...
                  <button className="flex items-center gap-1 text-sm font-medium rounded-full bg-[#FFFFFF3D] px-2 py-1 text-white focus:outline-none">
                    {mounted
                      ? languages.find((lang) => lang.code === currentLang)
                        ?.name || t("language")
                      : t("language")}
                    {mounted && languages?.length > 1 && <FaChevronDown size={10} />}
                  </button>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="mt-0.5 text-sm text-gray-600">
                          {mounted && location && location?.filter(Boolean)?.length > 0
                ...
                <ul className="hidden items-center gap-4 xl:flex">
                  <li className="hover:primaryColor font-medium text-gray-700">
                    <Link href={`/`} className="flex flex-col items-center" onClick={() => dispatch(setLockedFilter(null))}>
                      <span className={router.pathname === '/' ? 'primaryColor font-bold' : ''} aria-current={router.pathname === '/' ? 'page' : undefined}>
                        {t("home")}
                      </span>
                    </Link>
                  </li>
                          ...
                          <ul className="py-1 [&>li:last-child>button]:border-b-0">
                            {menu?.links?.map((link) => (
                              <li key={link.name}>
                                ...
                                >
                                  <span
                                    className="transition-all duration-150 group-hover:ml-2"
                                    aria-current={link.route && router?.pathname?.includes(link.route) && link.route !== '/' ? 'page' : undefined}
                                  >
