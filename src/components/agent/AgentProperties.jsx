...
import { formatExpiryDateLabel, handlePackageCheck, isRTL, RejectionTooltip, renderStatusBadge, truncate } from "@/utils/helperFunction";
...
const AgentProperties = () => {
    ...
    const openPayAsYouGoFlow = async ({ actionType, propertyId }) => {
        try {
            ...
            const propertyPayAsYouGo = payAsYouGoList.find(
                (item) => item.type === "property" && item.status === 1
            );
            setPayAsYouGoData(propertyPayAsYouGo || null);
        } catch (err) {
            console.error("Error fetching pay as you go data:", err);
            setPayAsYouGoData(null);
        }

        setPendingPayAsYouGoAction({ actionType, propertyId });
            ...
            renderCell: (elem) => (
                <div className="flex flex-col font-medium text-left">
                    <span className="text-sm">
                        {getDualPriceDisplay(elem?.price, elem?.currency).mainPriceString}
                    </span>
                    <span className="text-xs text-gray-500">{getDualPriceDisplay(elem?.price, elem?.currency).convertedPriceString}</span>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className='flex flex-col xs:flex-row gap-3'>
                    ...
                    <Select value={propertyTypeFilter} onValueChange={(value) => {
                        ...
                        <SelectContent>
                            <SelectItem value=" ">{t("propertyType")}</SelectItem>
