...
import { VerifiedAgentBadge, VerifiedUserBadge } from '@/utils/helperFunction';
...
const AppointmentTableRow = ({
    ...
    const isUserVerifiedAgent = user?.is_agent === true && user?.is_agent_verified === true;
    const isUserVerifiedUser = user?.is_user_verified === true;

    const isAgentVerifiedAgent = agent?.is_agent_verified === true;
    const isAgentVerifiedUser = agent?.is_user_verified === true;
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-8 p-4 w-full no-scrollbar">
                    ...
                    <div className="flex justify-start items-center gap-4 w-full md:w-[300px] md:max-w-[300px]">
                        ...
                        <div className="flex flex-col gap-1 w-full md:w-[220px] md:max-w-[220px]">
                            <span className="font-bold text-base text-gray-800 truncate text-start flex items-center gap-1">
                                {isAgent ? user?.name : agent?.name}
                                {isAgent ?
                                    isUserVerifiedAgent ? <VerifiedAgentBadge /> : isUserVerifiedUser ? <VerifiedUserBadge /> :
                                        null :
                                    isAgentVerifiedAgent ? <VerifiedAgentBadge /> : isAgentVerifiedUser ? <VerifiedUserBadge /> :
                                        null}
                            </span>
                    ...
                    {/* Property Info */}
                    {(isAgent || showPropertyInfo) && propertyDetails && (
                        <div className="flex justify-start items-center gap-4 w-full md:w-[300px] md:max-w-[300px]">
                            <div className="relative w-12 h-12 flex-shrink-0">
                                <ImageWithPlaceholder
