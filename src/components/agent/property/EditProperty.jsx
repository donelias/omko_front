...
const EditProperty = ({ params = [] }) => {
    ...
    const [listingRequestStatus, setListingRequestStatus] = useState("");
    ...
    const handleNewFileUpload = (mediaType, files) => {
        ...
        setMediaFormData(prev => ({
            ...
            [mediaType]: mediaType === 'titleImages' || mediaType === 'images3D'
                ? [singleFile] // Single file for title and 3D images
                : mediaType === 'documents' || mediaType === 'galleryImages'
                    ? [...prev[mediaType], ...normalizedFiles] // Multiple files for documents and gallery
                    : singleFile
        }));

        // Track newly uploaded files y
    ...
    const handleCheckRequiredFields = (currentTab, nextTab) => {
        ...
        // Validate all tabs from current to target when moving forward
        const tabsToValidate = nextTab
            ? tabOrder.slice(currentIndex, nextIndex)
            : [currentTab];

        for (const tabToValidate of tabsToValidate) {
            ...
            switch (tabToValidate) {
                ...
                // seoSettings has no required fields, skip validation
                case "seoSettings":
                default:
                    break;
            }
        ...
        // Handle final submission when on seoSettings tab and nextTab is null
        if (!missingFields && currentTab === "seoSettings" && nextTab === null) {
            handleEditProperty();
            return;
        }

        // Navigate to next tab if validation passed
    ...
    const handleFetchPropertyDetails = async () => {
        ...
        try {
            ...
            if (response && response.data && response.data.length > 0) {
                ...
                setListingRequestStatus(propertyData.request_status || "");
                ...
                // Reverse-map API numeric video_type → UI string token
                const videoTypeMap = { 1: 'youtubeLink', 2: 'vimeoLink', 0: 'customVideo' };
                const rawVideoType = propertyData.video_type;
                ...
                // Set title image directly from URL
                if (propertyData.title_image) {
                    setMediaFormData(prev => ({
                        ...prev,
                        titleImages: [{ url: propertyData.title_image, name: "title_image", isExisting: true }]
                    }));
                }

                // Set 3D image directly from URL
                if (propertyData.three_d_image) {
                    setMediaFormData(prev => ({
                        ...prev,
                        images3D: [{ url: propertyData.three_d_image, name: "3D_image", isExisting: true }]
                    }));
                }

                // Set gallery images directly from URLs
                if (propertyData.gallery && propertyData.gallery.length > 0) {
                    const galleryFiles = propertyData.gallery.map(galleryItem => ({
                        url: galleryItem.image_url,
                        id: galleryItem.id,
                        name: galleryItem.image || `gallery_${galleryItem.id}`,
                        isExisting: true
                    }));
                    setMediaFormData(prev => ({
                        ...prev,
                ...
                // Set SEO OG image URL directly — SEODetailsTab handles both File and string URL
                if (propertyData.meta_image) {
                    setSeoFormData(prev => ({
                        ...prev,
                        ogImage: propertyData.meta_image
                    }));
                }

                // Set documents directly from URLs
                if (propertyData.documents && propertyData.documents.length > 0) {
                    const documentFiles = propertyData.documents.map(doc => ({
                        url: doc.file,
                        id: doc.id,
                ...
                router.push(`${router?.asPath?.includes("/user") ? "/user/listings" : "/agent/dashboard"}?lang=${lang}`);
            }
        } catch (error) {
    ...
    const handleEditProperty = async (e) => {
        ...
        try {
            ...
            const isUntouchedCustomVideo = mediaFormData.videoType === 'customVideo' && !mediaFormData.customVideo && !shouldRemoveVideo;
            ...
            const data = {
                ...
                ...(!shouldRemoveVideo && !isUntouchedCustomVideo ? {
            ...
            if (response && !response.error) {
                ...
                router.push(router?.asPath?.includes("/user") ? `/user/listings?tab=properties&lang=${lang}` : `/agent/properties?lang=${lang}`);
            } else {
    ...
    // handler for generating ai property description
    ...
    // handler for generating ai meta data
        ...
        <>
            ...
            >
                ...
                {activeTab === "seoSettings" && (
                    <SEODetailsTab
                        showLoader={showLoader}
                        handleSubmit={listingRequestStatus === "draft" ? handleActivateProperty : () => handleEditProperty()}
                        handleTabChange={handleTabChange}
                        ...
                        submitButtonText={listingRequestStatus === "draft" ? t("activateProperty") : t("updateProperty")}
                    />
                )}
            </PropertyFormShell>
        </>
