...
const PropertiesOnMap = () => {
  ...
  const defaultCenter = useMemo(
    () => ({
      lat: Number(webSettings?.latitude) || 19.3835,
      lng: Number(webSettings?.longitude) || -70.2838,
    }),
    [webSettings],
  );
  ...
  const updateURLWithFilters = useCallback((newFilters) => {
    ...
    const newUrl = queryString ? `${currentPath}?${queryString}&lang=${lang}` : `${currentPath}?lang=${lang}`;

    router.push({
      pathname: `/properties-on-map/`,
      query: queryString ? { ...Object.fromEntries(urlParams.entries()), lang } : { lang },
    }, newUrl
    );
  }, [router]);
