...
import { getPostedSinceString, generateBase64FilterUrl, decodeBase64FilterUrl } from "@/utils/helperFunction";
...
const PropertiesOnMap = () => {
  ...
  // Helper function to initialize filters from the base64-encoded `filters` URL
  // param — same encode/decode scheme as PropertyList.jsx, so a shared link
  // format works across both pages.
  const initializeFiltersFromURL = useCallback(() => {
    ...
    if (query.filters) {
      try {
        ...
        return {
          propertyType: decodedFilters.property_type === 0 ? 'Sell' : decodedFilters.property_type === 1 ? 'Rent' : 'All',
          selectedCategory: decodedFilters.category_id || '',
          ...
          minPrice: decodedFilters.price?.min_price || '',
          maxPrice: decodedFilters.price?.max_price || '',
          postedSince: getPostedSinceString(decodedFilters.posted_since) || 'anytime',
          amenities: Array.isArray(decodedFilters.parameters) ? decodedFilters.parameters : [],
          nearbyPlaces: Array.isArray(decodedFilters.nearby_places) ? decodedFilters.nearby_places : [],
  ...
  useEffect(() => {
    if (router?.isReady) {
      ...
      setNearbyPlaces(urlFilters.nearbyPlaces);
  ...
  // Shared filter encoder — same base64 shape PropertyList.jsx produces, used
  // both for the address-bar URL and for the `filters` param sent to the API,
  // so there's a single source of truth instead of two separate manual builders.
  const encodeFilters = useCallback((f) => generateBase64FilterUrl({
    property_type: f.propertyType,
    category_id: f.selectedCategory,
    city: f.city,
    state: f.state,
    country: f.country,
    min_price: f.minPrice,
    max_price: f.maxPrice,
    posted_since: f.postedSince,
    keywords: f.keywords,
    amenities: f.amenities,
    nearbyPlaces: f.nearbyPlaces,
  }), []);

  // Function to update URL with current filter state, base64-encoded the same
  // way PropertyList.jsx does (generateBase64FilterUrl already drops falsy /
  // empty values, so no manual per-field filtering is needed here).
  const updateURLWithFilters = useCallback((newFilters) => {
    if (!router?.isReady) return;

    const base64Filters = encodeFilters(newFilters);
    ...
    router.push(
      ...
      `/properties-on-map/?${new URLSearchParams(query).toString()}`
    );
  }, [router, lang, encodeFilters]);
  ...
  const fetchProperties = async (isReset = false, loadMore = false) => {
    try {
      ...
      // Prepare API parameters — only the base64-encoded filters blob is sent,
      // same shape as the URL's `filters` param, no separate per-field params.
      const apiParams = {
        filters: encodeFilters({
          propertyType, selectedCategory, keywords, city, state, country,
          minPrice, maxPrice, postedSince, amenities, nearbyPlaces,
        }),
      };
  ...
  const handleClearFilters = () => {
    ...
    setNearbyPlaces(clearedFilters.nearbyPlaces);
