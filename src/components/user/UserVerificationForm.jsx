...
const UserVerificationForm = () => {
    ...
    const handleChange = (e, field) => {
        ...
        if (type === "file") {
            ...
            if (file) {
                ...
                setModifiedFiles((prev) => new Set(prev).add(name));
    ...
    const handleSubmit = async (e) => {
        ...
        const formFieldsData = formFields.map((field) => {
            ...
            // For file fields: only include if it's a newly uploaded file (File object)
            // Skip existing files (strings/URLs) that haven't been modified
            if (field.field_type === "file") {
                // Only include if:
                // 1. Field is in modifiedFiles set (user uploaded a new file)
                // 2. Value is a File object (not a string URL)
                if (modifiedFiles.has(field.name) && fieldValue instanceof File) {
                    return { id: field.id, value: fieldValue };
                }
                // Skip if it's an existing URL (string) or hasn't been modified
                if (typeof fieldValue === 'string') {
                    // This is an existing file URL from the server, don't send it again
                    return { id: field.id, value: null };
                }
                // Any other case for file fields
                return { id: field.id, value: null };
            }

            // For non-file fields, return the value as usual
            return { id: field.id, value: fieldValue };
        });
                                ...
                                field.form_fields_values.map((option, optionIndex) => (
                                    <div key={`checkbox_${option?.id}_${optionIndex}`}>
                                        ...
                                        <label
                                            ...
                                            className={`p-2 w-full h-10 flex items-center justify-center rounded border ${(formData[field.name] || []).includes(option.value) ? 'primaryBg text-white' : 'bg-white'
                            ...
                            <div className="file-input-container primaryBackgroundBg p-3">
                                <label className="cursor-pointer inline-flex items-center">
                                    ...
                                    <input
                                        ...
                                        name={field.name}
                                        onChange={(e) => handleChange(e, field)}
                                        accept=".jpg,.jpeg,.png,.gif,.pdf,.doc,.docx,.webp"
                                        className="hidden"
                                    />
                                </label>
                            </div>
