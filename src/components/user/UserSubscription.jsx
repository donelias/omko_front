...
const UserSubscription = () => {
    ...
    const fetchActivePackages = async () => {
        try {
            ...
            if (!response?.error) {
                const activePackages = Array.isArray(response?.active_packages) ? response?.active_packages : [];
                const allFeatures = Array.isArray(response?.all_features) ? response?.all_features : [];
                setActivePackages(activePackages);
                setAllFeatures(allFeatures);
            } else {
                console.error('Error fetching active packages:', response?.message);
                setActivePackages([]);
                setAllFeatures([]);
            }
            setIsLoading(false);
    ...
    }, [showPaystackModal, showPaypalModal, showFlutterwaveModal, showCashfree, showMidtrans, showPhonepe]);
