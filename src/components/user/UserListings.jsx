...
import { isRTL, renderStatusBadge, RejectionTooltip, formatExpiryDateLabel, handlePackageCheck, handleProjectAddCheck, handlePropertyAddCheck } from "@/utils/helperFunction";
...
const MyListings = () => {
            ...
            renderCell: (item) => item?.price !== "-" ? (
                <div className="flex flex-col font-medium text-left">
                    <span className="text-sm">
                        {getDualPriceDisplay(item?.price, item?.currency).mainPriceString}
                    </span>
                    <span className="text-xs text-gray-500">{getDualPriceDisplay(item?.price, item?.currency).convertedPriceString}</span>
    ...
    return (
        <div className="flex flex-col rounded-xl md:rounded-2xl border newBorderColor w-full h-full bg-white overflow-auto">
            ...
            <div className={`flex flex-col gap-4 h-full ${totalItems !== 0 ? 'max-h-[775px]' : ''}`}>
                            ...
                            {item?.price !== "-" && (
                                <div className="flex justify-between pb-2 border-b">
                                    ...
                                    <div className="text-right">
                                        <span className="block text-base font-bold primaryColor">{getDualPriceDisplay(item?.price, item?.currency).mainPriceString}</span>
                                        <span className="block text-xs text-gray-500">{getDualPriceDisplay(item?.price, item?.currency).convertedPriceString}</span>
                                    </div>
                                </div>
                            )}
