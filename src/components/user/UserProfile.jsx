...
const UserProfile = () => {
    ...
    const fetchAddressFromCoordinates = async (lat, lng) => {
        try {
            ...
            if (response?.error === false && response?.data?.result && response.data.result) {
                ...
                const formattedAddress = addressData.formattedAddress || firstResult.formatted_address || `${latitude.toFixed(6)}, ${longitude.toFixed(6)}`;
    ...
    const [formData, setFormData] = useState({
        ...
        phone: userData?.country_code && userData?.mobile ? `${userData?.country_code}${userData?.mobile}` : userData?.mobile || '',
    ...
    useEffect(() => {
        if (userData?.profile) {
            ...
            if (userData.profile.includes('googleusercontent.com')) {
                ...
                // Ensure relative URLs are properly formatted for ImageWithPlaceholder
                let profileUrl = userData.profile;
                if (profileUrl && !profileUrl.startsWith('http') && !profileUrl.startsWith('data:')) {
                    // If it doesn't start with /, add it (convert relative paths to absolute)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.data) {
                ...
                if (updatedData.profile && !updatedData.profile.startsWith('http') && !updatedData.profile.startsWith('data:')) {
                    ...
                    // If it contains /storage/, construct full path
                    else if (profile.includes('/storage/')) {
                        updatedData.profile = `/${profile}`;
                    }
                    // Otherwise it's just a filename, store as is for ImageWithPlaceholder to handle
                    else {
                        updatedData.profile = profile;
                    }
                    console.log('📸 Updated profile URL:', updatedData.profile);
                }
                dispatch(updateUserProfile({
                    data: updatedData
                }));
