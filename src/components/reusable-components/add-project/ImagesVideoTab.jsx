...
const ImagesVideoTab = ({
    ...
    const onDropTitleImage = useCallback(async (acceptedFiles) => {
        if (acceptedFiles.length > 0) {
            const file = acceptedFiles[0]; // Take only the first file
            const compressedFile = await compressImageFile(file, standardCompression);
            ...
            setMediaFormData(prev => ({
                ...prev,
                titleImage: compressedFile
            }));
        }
    }, [setMediaFormData, compressImageFile, standardCompression]);
    ...
    // Video type change handler — clears the opposing field
    ...
    }, [setMediaFormData, isEditing, setShouldRemoveVideo]);
    ...
    const galleryImageFiles = useMemo(() => {
        ...
        return mediaFormData.galleryImages.map((file, index) => {
            try {
                // Support string URLs, File objects and persisted objects with name/url fields.
                let imageUrl = "";
    ...
    return (
        <div className="flex flex-col gap-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="space-y-2">
                    ...
                    <Select value={videoType} onValueChange={(value) => handleVideoTypeChange(value)}>
                        ...
                        <SelectContent>
                            <SelectItem value="youtubeLink">{t("youtube")}</SelectItem>
                            <SelectItem value="vimeoLink">{t("vimeo")}</SelectItem>
                            {isCustomVideoUpload && (
                                <SelectItem value="customVideo">{t("customVideo")}</SelectItem>
                            )}
                        </SelectContent>
                    </Select>
                    ...
                    {videoType === 'customVideo' && isCustomVideoUpload && (
                        <div className="space-y-2">
                            ...
                            <div className="primaryBackgroundBg rounded-md">
                                <div
                                    {...getVideoRootProps()}
                                    className={`border border-dashed p-4 rounded-md cursor-pointer transition-colors duration-200 flex flex-col items-center justify-center min-h-[120px] ${
                                        isVideoDragActive ? 'border-primary bg-primary/5' : 'border-gray-300'
                                    }`}
                                >
            ...
            <div className="flex justify-end">
                <Button
                    onClick={() => {
                        handleCheckRequiredFields("imagesVideo", "location");
                    }}
