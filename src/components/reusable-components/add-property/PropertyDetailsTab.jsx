    ...
    return (
        <>
                        ...
                        <div className="flex gap-3 flex-col lg:flex-row">
                            ...
                            {propertyFormData.propertyType?.toLowerCase() === "rent" &&
                                <div className="flex flex-col w-full lg:flex-1">
                                    <Label htmlFor="property-rent-duration" className="text-nowrap font-medium mb-2 block text-gray-800">
                            ...
                            {/* Is Premium Property */}
                            {webSettings?.show_premium_toggle === "1" ? (
                                <div className="flex flex-col w-full lg:flex-1">
                                    ...
                                    <div className="flex items-center">
                                        <Switch
                                            checked={propertyFormData.isPremiumProperty}
                                            name="isPremiumProperty"
                                            onCheckedChange={(checked) => handleUpdatePropertyForm(null, "isPremiumProperty", checked)}
                                            className={`${propertyFormData.isPremiumProperty ? "!primaryBg" : ""}  sm:[&>span]:h-3 sm:[&>span]:w-3 ${isRtl ? "sm:[&>span]:data-[state=checked]:-translate-x-5" : "sm:[&>span]:data-[state=checked]:translate-x-5"}`}
                                        />
                                    </div>
                                </div>) : <div className="w-full lg:flex-1"></div>}
                        </div>
                    )}
                </div>
                {/* Property Description */}
                <div className='flex flex-col'>
                    {webSettings?.gemini_ai_enabled === true ? (
                        <div className='flex justify-between items-center mb-2'>
                            ...
                            <button
                                disabled={isDescriptionGenerating || propertyFormData.propertyTitle.trim() === ""}
                                onClick={handleGenerateAIDescription}
                                className='flex justify-between items-center gap-2 p-1 md:p-2 border primaryBorderColor rounded-md primaryBgLight08 font-medium text-sm primaryColor '
                            >
                                <RiSparkling2Fill className='primaryColor w-5 h-5' /> <span className=''>{isDescriptionGenerating ? t("generating") : t("generateWithAI")}</span>
                            </button>
                        </div>
                    ) : (
                        <Label htmlFor="property-description" className="font-medium mb-2 block text-gray-800">
                    ...
                    {isDescriptionGenerating ?
                        (
                            <Skeleton className={"h-[250px] flex px-3 py-2 w-full primaryBackgroundBg rounded-md"} >
                                {t("generating")}
                            </Skeleton>
                        ) :
                        (
                            <Textarea
                                ...
                                className="h-[250px] w-full px-3 py-2 rounded-md focus:outline-none focus:border-none primaryBackgroundBg focus:border-transparent"
                            />
                        )}
                </div>
            </div>
            ...
            <div className="flex justify-end" >
                <Button
                    onClick={() => handleCheckRequiredFields("propertyDetails", "imagesVideo")}
