...
const ImagesVideoTab = ({
    ...
    const panoramaCompression = { maxSizeMB: 1, maxWidthOrHeight: 4096, initialQuality: 0.92 };
    ...
    const onDropTitleImage = useCallback(async (acceptedFiles) => {
        if (acceptedFiles.length > 0) {
            ...
            if (isEditing && handleNewFileUpload) {
                handleNewFileUpload('titleImages', [compressedFile]);
            } else {
                setMediaFormData(prev => ({
                    ...prev,
                    titleImages: [compressedFile]
                }));
            }
        }
    }, [isEditing, handleNewFileUpload, setMediaFormData, compressImageFile, standardCompression]);
    ...
    // 3D Image Handler
    const onDrop3DImage = useCallback(async (acceptedFiles) => {
        if (acceptedFiles.length > 0) {
            const file = acceptedFiles[0];
            const compressedFile = await compressImageFile(file, panoramaCompression);

            if (isEditing && handleNewFileUpload) {
                handleNewFileUpload('images3D', [compressedFile]);
            } else {
                setMediaFormData(prev => ({
                    ...prev,
                    images3D: [compressedFile]
                }));
            }
        }
    }, [isEditing, handleNewFileUpload, setMediaFormData, compressImageFile, panoramaCompression]);
