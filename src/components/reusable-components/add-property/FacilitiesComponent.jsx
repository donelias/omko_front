...
const FacilitiesComponent = ({
    ...
    useEffect(() => {
        if (selectedCategory) {
            // Find the selected category data from categories in Redux, or use selectedCategory directly
            const category = categories?.find
                ? categories.find(cat => cat.id === selectedCategory?.id)
                : selectedCategory;
            const source = category || selectedCategory;
            if (source) {
                const params = typeof source.parameter_types === "object"
                    ? (Array.isArray(source.parameter_types) ? source.parameter_types : Object.values(source.parameter_types))
                    : source.parameter_types;
                setParameterTypes(params);

                // Check for existing file values in formData and set in uploadedFiles
                if (isEditing && Object.keys(formData).length > 0 && params) {
                    const fileParams = params.filter(param => param.type_of_parameter === 'file');
    ...
    const filteredParameterTypes = useMemo(() => {
        if (!parameterTypes) return null;
        if (!filterType) return parameterTypes;
        return parameterTypes.filter(p => p.type === filterType);
    }, [parameterTypes, filterType]);
    ...
    return (
        <>
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {filteredParameterTypes && filteredParameterTypes.map((parameter) => (
                    <div key={parameter.id} className="space-y-2">
                        ...
                        {parameter.type_of_parameter === "checkbox" && (
                            <div className="flex flex-wrap items-start gap-2">
                                {parameter.translated_option_value && parameter.translated_option_value.length > 0 ? (
                                    parameter.translated_option_value.map((option, index) => (
                                        <label
                                    ...
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            type="checkbox"
                                            name={parameter.id}
                                            checked={formData[parameter.id] === "1" || formData[parameter.id] === true}
                                            onChange={(e) => setFormData(prev => ({ ...prev, [parameter.id]: e.target.checked ? "1" : "0" }))}
                                            className="w-4 h-4"
                                        />
                                        <span className="text-sm">{t("yes") || "Yes"}</span>
                                    </label>
                                )}
                            </div>
                        )}
