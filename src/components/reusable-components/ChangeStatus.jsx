    ...
    <div className={"mb-4 w-full rounded-2xl border bg-white"}>
      ...
      <div className="space-y-4 p-4">
        <div>
          <label
            htmlFor="status"
            className="secondaryTextColor mb-2 block text-sm font-normal"
          >
            {t("status")}
          </label>
          <Select value={status} onValueChange={handleStatusChange}>
            <SelectTrigger className="!secondaryTextColor h-10 w-full rounded-md border bg-white focus:ring-0 focus:ring-offset-0">
              <SelectValue placeholder="Select status" />
            </SelectTrigger>
            <SelectContent>
              {statusOptions.map((option) => (
                <SelectItem
                  className="hover:!primaryBgLight hover:!secondaryTextColor hover:cursor-pointer"
                  key={option?.value}
                  value={option?.value}
                >
                  {option?.label}
