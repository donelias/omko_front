...
import { getDisplayValueForOption, truncate } from '@/utils/helperFunction';
...
const AppointmentPropertyCard = ({
    ...
    const { mainPriceString, convertedPriceString } = getDualPriceDisplay(propertyData?.price, propertyData?.currency);
                            ...
                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-1 md:gap-0 mb-2">
                                ...
                                <div className="flex items-center gap-2 flex-shrink-0">
                                    <div className="text-right">
                                        <span className="block text-lg md:text-xl font-bold primaryColor">
                                            {mainPriceString} <span className='text-sm font-normal'>{propertyData?.property_type === "rent" && propertyData?.rentduration ? "/ " + propertyData?.rentduration : ""}</span>
                                        </span>
                                        <span className="block text-xs md:text-sm text-gray-500 font-medium">{convertedPriceString}</span>
                                    </div>
                                </div>
                            </div>
