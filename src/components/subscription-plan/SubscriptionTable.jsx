...
export const SubscriptionTable = ({ packages = [], features = [], currencySymbol = "" }) => {
    ...
    return (
        <div className="container mx-auto pt-12 pb-20">
            <div className="hidden overflow-hidden rounded-xl border newBorderColor bg-white shadow-sm md:block" role="table" aria-label="Subscription Comparison Table">
                <div className="w-full pb-0">
                    <div className="w-full">
                        <div className="flex border-b newBorderColor" role="row">
                            <div className="w-1/4 shrink-0 border-r newBorderColor bg-white p-3 md:p-6" role="columnheader" />
                            {activePackages.map((pkg, idx) => (
                                <div
                                    key={pkg?.id ?? idx}
                                    className={`flex-1 shrink-0 border-r newBorderColor p-3 text-center md:p-6 ${idx < activePackages.length - 1 ? "" : "border-r-0"}`}
                                    role="columnheader"
                                >
                        ...
                        <div className="flex flex-col" role="rowgroup">
                            {activeFeatures.map((feature, featureIndex) => (
                                <div
                                    ...
                                    className={`flex w-full transition-colors hover:bg-gray-50 ${featureIndex < activeFeatures.length - 1 ? "border-b newBorderColor" : ""}`}
                                    role="row"
                                >
                                    <div className="flex w-1/4 shrink-0 items-center border-r newBorderColor p-3 text-xs font-bold brandColor sm:text-sm md:p-6 md:text-base" role="rowheader">
                                        {feature?.translated_name || feature?.name}
                                    </div>
                                    {activePackages.map((pkg, packageIndex) => (
                                        <div
                                            key={`${pkg?.id ?? packageIndex}-${feature?.id ?? featureIndex}`}
                                            className={`flex flex-1 items-center justify-center border-r newBorderColor p-3 text-center md:p-6 ${packageIndex < activePackages.length - 1 ? "" : "border-r-0"}`}
                                            role="cell"
                                        >
            ...
            <div className="md:hidden">
                <Carousel opts={carouselOptions} className="relative w-full">
                    ...
                    <div className="mt-4 flex items-center justify-center gap-3 px-2">
                        <CarouselPrevious className="static h-9 w-9 translate-x-0 translate-y-0 newBorder bg-white brandColor shadow-none" />
                        <CarouselDots className="min-w-0 justify-center px-0 !overflow-x-visible" />
                        <CarouselNext className="static h-9 w-9 translate-x-0 translate-y-0 newBorder bg-white brandColor shadow-none" />
                    </div>
                </Carousel>
            </div>
        </div>
