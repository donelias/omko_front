        ...
        {!showBankDetails ? (
          ...
          <div className="p-6 flex flex-col gap-4 ">
            <DialogHeader className="space-y-2 pb-3 border-b">
              <DialogTitle className="flex items-center justify-between text-xl md:text-2xl font-semibold brandColor">
                {t("selectPaymentMethod")}
                <button
                  onClick={onClosePaymentSelectionModal}
                  className="primaryBackgroundBg flex h-9 w-9 items-center justify-center rounded-lg transition-colors hover:opacity-80"
                  aria-label="Close"
                >
            ...
            <div className="flex flex-col gap-4 ">
              ...
              {availableOnlinePayments.length > 0 && (
                <div className="flex flex-col gap-4">
                  <div className="flex items-center gap-2">
                    <IoMdCard className="primaryColor h-7 w-8" />
                    <h3 className="text-lg font-semibold brandColor">
                      {t("onlinePayments")}
                    </h3>
                  </div>
                  <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                    {availableOnlinePayments.map((method) => (
                      <button
                        ...
                        className={cn(
                          "relative flex h-32 flex-col items-center justify-center rounded-xl border-2 p-4 transition-all duration-200 ",
                          selectedPaymentMethod === method.id && selectedPaymentType === "online"
                            ? "primaryBorderColor primaryBgLight"
                            : "border-gray-200 bg-white hover:bg-gray-50"
                        )}
                      >
                        {/* Selection indicator */}
                        <div className={`primaryBg absolute right-2 top-2 flex h-5 w-5 items-center justify-center rounded-full opacity-0 transition-opacity duration-200 pointer-events-none ${selectedPaymentMethod === method.id && selectedPaymentType === "online" ? "opacity-100" : "opacity-0"}`}>
                          <FaCheckCircle className="h-4 w-4 text-white" />
                        </div>

                        <div className="mb-3 flex h-12 w-12 flex-shrink-0 items-center justify-center">
                          <Image
                            src={method.icon}
                            alt={method.name}
                            width={48}
              ...
              {availableOfflinePayments.length > 0 && (
                <div className="flex flex-col gap-4">
                  <div className="flex items-center gap-2">
                    <RiBankFill className="primaryColor h-7 w-8" />
                    <h3 className="text-lg font-semibold text-gray-800">
                      {t("offlinePayments")}
                    </h3>
                  </div>
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    {availableOfflinePayments.map((method) => (
                      <button
                        ...
                        className={cn(
                          "relative flex h-20 items-center rounded-xl border-2 p-4 transition-all duration-200 hover:border-gray-400",
                      ...
                      >
                        {/* Selection indicator */}
                        <div className={`primaryBg absolute right-3 top-3 flex h-5 w-5 items-center justify-center rounded-full opacity-0 transition-opacity duration-200 pointer-events-none" ${selectedPaymentMethod === method.id && selectedPaymentType === "offline" ? "opacity-100" : "opacity-0"}`}>
                          <FaCheckCircle className="h-4 w-4 text-white" />
                        </div>

                        <div className="primaryColor mr-4 flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-lg bg-gray-100">
                          {method.icon}
                        </div>
            ...
            <div className="flex items-center justify-end space-x-3 border-t border-gray-200 pt-4">
              <Button
                onClick={onClosePaymentSelectionModal}
                variant="outline"
                className="rounded-lg border-2 border-gray-300 px-6 py-2 font-medium text-gray-700 transition-colors hover:bg-gray-50"
              >
                {t("cancel")}
              </Button>
              <Button
                ...
                className="primaryBg rounded-lg px-6 py-2 font-medium text-white transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {t("continue")}
              </Button>
            </div>
