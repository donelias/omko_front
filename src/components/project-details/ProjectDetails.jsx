...
import ProjectInventoryWidget from "./ProjectInventoryWidget";

const ProjectDetails = ({ initialData, seoData }) => {
  const router = useRouter();
  const { slug, lang } = router.query;
  const t = useTranslation();
  const language = useSelector((state) => state.LanguageSettings?.active_language);
  // When getServerSideProps delivered the project, render it immediately
  // (this is what search engines see) and skip the duplicate client fetch.
  const initialProject = initialData?.data || null;
  const [projectDetails, setProjectDetails] = useState(initialProject);
  const [loading, setLoading] = useState(!initialData);
  const [error, setError] = useState(null);
  const [similarProjects, setSimilarProjects] = useState(initialData?.similar_projects || []);
  ...
  const isPremiumProject = projectDetails?.is_premium;
  const [isPremiumProjectUser, setIsPremiumProjectUser] = useState(null);
  ...
  useEffect(() => {
    ...
    const serverProject = initialData?.data;
    if (serverProject && serverProject.slug_id === slug) {
      setProjectDetails(serverProject);
      setSimilarProjects(initialData?.similar_projects || []);
      setLoading(false);
      setError(null);
      return;
    }
    ...
    fetchProjectDetails();
  }, [slug, language, initialData]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const fetchProjectDetails = async () => {
    ...
    try {
      ...
      if (response && !response.error) {
        ...
        if (response.data?.id && trackedProjectIdRef.current !== response.data.id) {
          trackedProjectIdRef.current = response.data.id;
  ...
  useEffect(() => {
    if (!isPremiumProject) return; // only check for premium projects

    const runPremiumCheck = async () => {
      const hasAccess = await checkPackageAvailable(PackageTypes.PREMIUM_PROJECTS);
      setIsPremiumProjectUser(hasAccess);
    };
  ...
  const handleProjectDirectLinkHit = () => {
    ...
    if (isPremiumProject && isPremiumProjectUser === false) {
      Swal.fire({
        title: t("oops"),
  ...
  useEffect(() => {
    // Only fire once the API has responded (isPremiumProjectUser is no longer null)
    if (slug && isPremiumProject && isPremiumProjectUser !== null) {
      handleProjectDirectLinkHit();
    }
  }, [slug, userCurrentId, isPremiumProjectUser]);
  ...
  return (
    <section className={`${isPremiumProject && !isPremiumProjectUser ? "blur-md" : ""}`}>
        ...
        <div className="container mx-auto px-3 pt-8">
          <div className="grid grid-cols-12 gap-3">
            ...
            <div className="col-span-12 h-full w-full lg:col-span-3">
              ...
              {/* On-plan inventory widget (units available to reserve) */}
              {projectDetails && (
                <ProjectInventoryWidget project={projectDetails} />
              )}
            </div>
          </div>
        </div>
