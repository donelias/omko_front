...
import PreQualificationModal from "../pre-qualification-modal/PreQualificationModal";
...
const ProjectDetails = () => {
  ...
  const isAuthenticated = useSelector((state) => !!state.User?.jwtToken);

  const handleReserveClick = (plan) => {
    if (!isAuthenticated) {
      showLoginSwal("oops", "plzLoginFirst", () => {
  ...
  const currentUrl = `${process.env.NEXT_PUBLIC_WEB_URL}/project-details/${slug}?share=true&lang=${lang}`;
  ...
  const isAccessible = !projectDetails?.is_premium;
  ...
  const handleCheckPremiumUserAgent = (e) => {
    e.preventDefault();
    router.push(
      `/agent-details/${projectDetails?.customer?.slug_id}?lang=${lang}`,
    );
  };
            ...
            <div className="col-span-12">
              ...
              {galleryPhotos && (
                <ProjectGallery
                  ...
                  blurDataURL={projectDetails?.low_quality_title_image}
                  isProject={true}
                  onImageClick={openLightbox}
                  videoLink={projectDetails?.video_link}
                  videoType={projectDetails?.video_type}
                />
              )}
            </div>
          </div>
        ...
        <div className="container mx-auto px-3 pt-8">
          <div className="grid grid-cols-12 gap-3">
            ...
            <div className="col-span-12 h-full w-full rounded-2xl lg:col-span-9">
              ...
              {projectDetails && projectDetails?.plans && projectDetails.plans.length > 0 && (
                <div className="cardBg newBorder mb-7 flex flex-col rounded-2xl">
                  <div className="blackTextColor border-b p-5 text-base font-bold md:text-xl">
                    {t("inventoryAvailability")}
                  </div>
                  <div className="p-5">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      {projectDetails.plans.map((plan) => {
                        const status = plan?.unit_status;
                        const available = parseInt(plan?.available_units) || 0;
                        const total = parseInt(plan?.total_units) || 0;
                        const isSoldOut = status === "sold_out" || available <= 0;
                        const isLowStock = status === "low_stock" || (available > 0 && available <= 3);

                        return (
                          <div
                            key={plan.id || plan.title}
                            className={`rounded-xl border p-4 transition hover:shadow-md ${
                              isSoldOut ? "border-red-200 bg-red-50/30" : isLowStock ? "border-amber-200 bg-amber-50/30" : "border-green-200 bg-green-50/30"
                            }`}
                          >
                            <div className="flex items-start justify-between mb-3">
                              <div>
                                <h4 className="font-semibold text-base">{plan.title}</h4>
                                {plan.unit_code && <p className="text-xs text-gray-400">{plan.unit_code}</p>}
                              </div>
                              <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${
                                isSoldOut ? "bg-red-100 text-red-700" : isLowStock ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"
                              }`}>
                                {isSoldOut ? <AlertTriangle size={12} /> : isLowStock ? <Clock size={12} /> : <CheckCircle size={12} />}
                                {isSoldOut ? t("soldOut") : isLowStock ? t("fewLeft") : t("available")}
                              </span>
                            </div>
                            <div className="flex items-center justify-between text-sm mb-3">
                              <div>
                                <span className="text-gray-400">{t("price")}: </span>
                                <span className="font-semibold">{plan?.price ? `${plan?.currency || "DOP"} ${plan?.price}` : "-"}</span>
                              </div>
                              <div>
                            ...
                            >
                              <ShoppingBag size={16} />
                              {isSoldOut ? t("unavailable") : t("reserve")}
                            </button>
                          </div>
                        );
                      })}
            ...
            <div className="col-span-12 h-full w-full lg:col-span-3">
              ...
              {/* Sidebar Inventory */}
              {projectDetails?.plans && projectDetails.plans.length > 0 && (
                <div className="cardBg newBorder mt-6 rounded-2xl">
                  <div className="blackTextColor border-b p-4 text-sm font-bold">
                    {t("inventoryAvailability")}
                  </div>
                  <div className="divide-y p-3">
                    {projectDetails.plans.map((plan) => {
                      const available = parseInt(plan?.available_units) || 0;
                      const total = parseInt(plan?.total_units) || 0;
                      const isSoldOut = plan?.unit_status === "sold_out" || available <= 0;
                      return (
                        <div key={plan.id || plan.title} className="flex items-center justify-between py-2">
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium truncate">{plan.title}</p>
                            <p className="text-xs text-gray-400">{available}/{total} {t("units")}</p>
                          </div>
                          <button
                            onClick={() => handleReserveClick(plan)}
                            disabled={isSoldOut}
                            className={`ml-2 shrink-0 rounded-lg px-3 py-1.5 text-xs font-medium transition ${
                              isSoldOut
      ...
      {showPreQualModal && (
        <PreQualificationModal
          ...
          onClose={() => {
            ...
            setSelectedPlanForReserve(null);
          }}
