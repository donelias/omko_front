...
import { capitalizeFirstLetter, getDisplayValueForOption } from "@/utils/helperFunction";
...
const PropertyInfoBanner = ({ property }) => {
  ...
  const renderDualPrices = () => {
    const { mainPriceString, convertedPriceString } = getDualPriceDisplay(
      property?.price,
      property?.currency,
    );
      ...
      <div className="text-right flex flex-col items-end leading-tight">
        <span className="brandColor text-sm font-bold sm:text-base md:text-xl lg:text-2xl">
          ...
          {property?.property_type === "rent" && property?.rentduration && (
            <span className="ml-1 text-sm font-normal text-gray-500">/{property?.rentduration}</span>
          )}
        </span>
        <span className="text-xs md:text-sm text-gray-400 font-medium block mt-1 tracking-wide">
  ...
  return (
    <div className="cardBg rounded-xl p-4 shadow-sm md:p-4 lg:p-6">
      <div className="flex flex-col md:flex-row items-center justify-between gap-2 border-b pb-4 md:gap-6">
        ...
        <div className="flex flex-col gap-3 w-full">
          ...
          <div className="mb-2 flex items-center gap-2 md:gap-4">
            <div className="primaryBackgroundBg flex items-center justify-center rounded-lg px-4 py-2">
              {property?.category?.image ? (
                ...
                <div className="h-5 w-5 rounded-full bg-gray-200 animate-pulse shrink-0" />
              )}
              {(property?.category?.translated_name || property?.category?.category || property?.category?.name) && (
          ...
          <h2 className="blackTextColor text-xl font-bold sm:text-base md:text-xl lg:text-2xl">
        ...
        <div className="w-full md:w-fit flex flex-col items-end self-start">
          <div className="mb-2 w-full md:w-fit flex flex-row-reverse md:flex-col justify-between md:justify-normal items-center gap-1 md:gap-3">
            <div className="flex justify-center items-center gap-2">
              <span className="hidden md:block text-nowrap leadColor text-sm font-medium">{t("propertyId")} : {property?.id}</span>
              <span className={`status-badge self-end ${property?.property_type === "sell" ? "primarySellBg primarySellText" : "primaryRentBg primaryRentText"} mr-3`}>
      ...
      {/* Footer - Ubicación y Parámetros */}
      <div className="flex flex-wrap items-center justify-center gap-2 pt-4 text-xs font-semibold sm:justify-between md:text-sm">
        <div className="leadColor flex items-center text-sm">
          <LuMapPin className="mr-1 text-lg shrink-0" />
        ...
        <div className="place-self-center flex flex-wrap justify-center md:justify-normal items-center gap-4 md:gap-4 [&>div:last-child>span]:border-0">
          {property?.parameters?.slice(0, 4)?.map((parameter) => (
            <div key={parameter?.id} className="flex items-center gap-2 md:gap-3">
              {parameter?.image ? (
                <ReactSVG
                  src={parameter.image}
                  beforeInjection={(svg) => {
                  ...
                  alt={parameter?.translated_name || parameter?.name || 'facilities icon'}
                />
              ) : (
                <div className="h-4 w-4 rounded-full bg-gray-200 animate-pulse shrink-0" />
              )}
              <span className="text-sm truncate font-medium">{getDisplayValueForOption(parameter)}</span>
