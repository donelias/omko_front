      ...
      <DialogContent className="!max-h-[90vh] w-full max-w-[95vw] overflow-y-auto p-0 sm:max-w-3xl md:max-w-4xl lg:max-w-5xl [&>button]:!hidden">
        ...
        <DialogFooter className="p-4">
          {totalEmiYearlyData && totalEmiYearlyData.length > 0 ? (
            ...
            <div className="flex w-full flex-col items-center justify-between gap-4 rounded-lg primaryBgLight08 p-4 sm:flex-row sm:gap-0">
              <div className="text-center sm:text-left">
                <h4 className="font-medium brandColor">
                  {t("accessMortgageCalculationTable")}
                </h4>
                <p className="text-sm text-gray-500">{t("viewTable")}</p>
              </div>
              <Button
                onClick={handleSubscribe}
                className="w-full rounded-md brandBg px-6 py-2 text-white transition-colors hover:primaryBg sm:w-auto"
              >
                {t("subscribeNow")}
              </Button>
            </div>
