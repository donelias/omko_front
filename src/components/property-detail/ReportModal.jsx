    ...
    <Dialog open={open} onOpenChange={handleReportModal}>
      <DialogContent className="!max-w-[300px] rounded-md p-0 sm:!max-w-md md:rounded-lg [&>button]:hidden">
        ...
        <div className={`p-4 ${reportReasons?.length > 0 ? "h-[500px] overflow-y-auto" : "h-fit"} md:max-h-full`}>
          {reportReasons?.length > 0 && <div className="secondaryTextColor mb-2 text-base">{t("reasons")}</div>}
            ...
            {loading ? (
              ...
              <>
                {reportReasons?.map((reason) => (
                  <div
                    key={reason?.id}
                    className={`${selectedReason === reason?.id ? "primaryCheckbox ring-1 ring-black" : ""} text-sm md:text-base secondaryTextColor newBorder flex items-center justify-between rounded-lg px-2 py-2 md:py-3 transition-colors duration-300 hover:cursor-pointer md:rounded-2xl`}
                    onClick={() => handleSelectReason(reason)}
                  >
                    <span className="break-all">{reason?.translated_reason || reason?.reason}</span>
                    <input
                      type="radio"
                      name="reportReason"
                ...
                <div
                  key={reason?.id}
                  className={`${selectedReason === 0 ? "primaryCheckbox ring-1 ring-black" : ""} text-sm md:text-base secondaryTextColor newBorder flex justify-center rounded-lg  px-2 py-2 md:py-3 transition-colors duration-300 hover:cursor-pointer md:rounded-2xl`}
          ...
          {selectedReason === 0 && (
            <>
              <div className="secondaryTextColor my-2 text-sm md:text-base">
                {t("writeReason")}
              </div>
