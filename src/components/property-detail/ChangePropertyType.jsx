    ...
    return (
        <div className={"w-full rounded-2xl border bg-white mb-4"}>
            ...
            <div className="p-4 space-y-4">
                <div>
                    <label htmlFor="status" className="block text-sm font-normal secondaryTextColor mb-2">
                        {t("status")}
                    </label>
                    <Select defaultValue={"0"} value={status} onValueChange={handleStatusChange}>
                        <SelectTrigger
                            className="w-full !secondaryTextColor bg-white border rounded-md h-10 focus:ring-0 focus:ring-offset-0">
                            <SelectValue placeholder="Select Property Type" />
                        </SelectTrigger>
                        <SelectContent>
                            {statusOptions.map((option) => (
                                <SelectItem className="hover:!primaryBgLight hover:!secondaryTextColor hover:cursor-pointer" key={option?.value} value={option?.value}>
                                    {option?.label}
                                </SelectItem>
                            ))}
                        </SelectContent>
