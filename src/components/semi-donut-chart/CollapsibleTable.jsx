          ...
          {data.map((row, rowIndex) => (
            <React.Fragment key={rowIndex}>
              ...
              >
                ...
                <TableCell
                  className={`secondaryTextColor text-base font-medium ${getAlignmentClass("left")} ${columns[1].className || ""}`}
                >
                  {row.year}
                </TableCell>
                <TableCell
                  className={`secondaryTextColor text-base font-medium ${getAlignmentClass("left")} ${columns[2].className || ""}`}
                >
                  {formatPriceAbbreviated(row.principal_amount)}
                </TableCell>
                <TableCell
                  className={`secondaryTextColor text-base font-medium ${getAlignmentClass("left")} ${columns[3].className || ""}`}
                >
                  {formatPriceAbbreviated(row.interest_paid)}
                </TableCell>
                <TableCell
                  className={`secondaryTextColor text-base font-medium ${getAlignmentClass("left")} ${columns[4].className || ""}`}
                >
                        ...
                        <tbody>
                          {row.monthly_totals.map((monthData, monthIndex) => (
                            ...
                            >
                              ...
                              <td className="secondaryTextColor w-[25%] py-2 pl-4 text-center text-base font-medium capitalize">
                                {t(monthData?.month?.toString().toLowerCase())}
                              </td>
                              <td className="secondaryTextColor w-[25%] py-2 text-center text-base font-medium">
                              ...
                              <td className="secondaryTextColor w-[20%] py-2 text-center text-base font-medium">
                                {formatPriceAbbreviated(
