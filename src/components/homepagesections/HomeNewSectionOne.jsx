...
import { isRTL, getLocationLatLngFilter } from '@/utils/helperFunction';
...
const HomeNewSectionOne = ({ translated_title, title, data = [], buttonText = "Explore More Listings", buttonLink = "/properties", name, passLocationFilter }) => {
    ...
    const userPreferedLocation = useSelector((state) => state.location?.city); // Added optional chaining

    const sectionTitle = name === "nearby_properties_section" && userPreferedLocation
        ? `${translated_title || title} ${userPreferedLocation}` // {title} in {userPreferedLocation}; in will be included in dynamic translated title
    ...
    const renderMobileCarousel = () => {
        if (sectionType === "user_recommendations_section") {
                    ...
                    >
                        <CarouselContent>
                            {displayData.slice(0, 4).map((element) => (
                                <CarouselItem key={element?.id} className={`basis-full sm:basis-1/2 md:basis-1/3 ${isMobile ? "flex justify-center" : ""}`}>
            ...
            const gridItems = sectionType === "categories_section" ? displayData : displayData.slice(0, 10);
            return (
                <Carousel
                    className="w-full"
                    plugins={isMobile && displayData?.length > 1 ? [autoplayPlugin.current] : []} // Only add autoplay plugin on mobile and when data length > 1
                        ...
                        {gridItems?.map((element) => (
                            <CarouselItem key={element.id} className={`${sectionType === "categories_section" ? "basis-1/2 md:basis-1/3 lg:basis-1/4" : "basis-[85%] sm:basis-1/2 md:basis-1/3 lg:basis-1/4 " + (isMobile ? "flex justify-center" : "")} `}>
                                {sectionType === "featured_projects_section" ? (
                                    ...
                                    <CategoryCard category={element} passLocationFilter={passLocationFilter} />
                                ) : (
                                    <PropertyVerticalCard property={element} />
                                )}
                            </CarouselItem>
    ...
    const renderDesktopGrid = () => {
        if (sectionType === "user_recommendations_section") {
            ...
            return (
                <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6 place-content-center">
                    {gridItems?.map((element) => {
                        ...
                        if (sectionType === "categories_section") {
                            return <CategoryCard category={element} key={element?.id} passLocationFilter={passLocationFilter} />;
                        } else {
    ...
    return (
        <div className={`${bgColor}`}> {/* Use the looked-up color */}
            <div className='container mx-auto px-2 md:px-0 py-5 md:py-10 lg:py-[60px]'>
                <div className='flex flex-col gap-6 md:gap-12 2xl:gap-8 items-center'>
                    ...
                    {/* Explore More Button */}
                    {buttonText && buttonLink && name !== "user_recommendations_section" && (displayData.length > 1) && (
                        <div className="text-center">
                            <Link
                                href={
                                    name === "nearby_properties_section"
                                        ? `${buttonLink}${passLocationFilter ? getLocationLatLngFilter("", "") : ""}`
                                        : buttonLink
                                }
                                onClick={() => {
                                    if (name === "nearby_properties_section") {
                                        dispatch(setLockedFilter("location"));
                                    }
                                }}
                                className="inline-flex items-center gap-2 py-2 px-3 md:px-4 md:py-3 border border-black rounded-md text-black font-medium hover:brandBg hover:text-white transition-colors duration-200"
                            >
                                {t(buttonText)}
                                <ArrowRight size={16} className={`${isRtl ? "rotate-180" : ""}`} />
                            </Link>
