...
const PropertyVerticalCard = dynamic(() => import("@/components/cards/PropertyVerticalCard"));
const PropertyCityCard = dynamic(() => import("../cards/PropertyCityCard"));
const ProjectCardWithSwiper = dynamic(() => import("../cards/ProjectCardWithSwiper"));
const CityCard = dynamic(() => import("../cards/CityCard"));
...
import { getBgColorConfig, getLocationLatLngFilter, getProjectFilters, isRTL } from "@/utils/helperFunction";
  ...
  const handleLink = () => {
    switch (type) {
      case "nearby_properties_section":
        return `/properties/properties-nearby-city?lang=${router?.query?.lang}`;
      case "properties_by_cities_section":
        return `/properties/properties-nearby-city?lang=${router?.query?.lang}`;
      case "projects_section":
        return `/projects` + (passLocationFilter ? getProjectFilters(router?.query?.lang) : `?lang=${router?.query?.lang}`);
      case "most_viewed_properties_section":
        return `/properties/most-viewed-properties` + (passLocationFilter ? getLocationLatLngFilter("", "", router?.query?.lang) : `?lang=${router?.query?.lang}`);
      case "most_liked_properties_section":
        return `/properties/most-favourite-properties` + (passLocationFilter ? getLocationLatLngFilter("", "", router?.query?.lang) : `?lang=${router?.query?.lang}`);
      default:
        return `/properties` + (passLocationFilter ? getLocationLatLngFilter("", "", router?.query?.lang) : `?lang=${router?.query?.lang}`);
    }
  };
  const getCarouselClassname = () => {
    if (type === "properties_by_cities_section" && !with_image) {
      return "sm:basis-1/2 md:basis-[36%] lg:basis-[27%] xl:basis-[22%] 2xl:basis-1/5 3xl:basis-1/6";
    } else if (type === "properties_by_cities_section" && with_image) {
      return "basis-full sm:basis-1/2 md:basis-1/3 lg:basis-1/4 xl:basis-1/5";
    } else if (type === "projects_section" || type === "similar_projects_section") {
      return "basis-full sm:basis-1/2 md:basis-1/3 lg:basis-1/4 xl:basis-1/5";
    }
    return "basis-full sm:basis-1/2 md:basis-1/3 lg:basis-1/4 xl:basis-1/5";
  }
  return (
    <div className={`${bgColor}`}>
      <div className="container mx-auto px-4 md:px-0 py-5 sm:py-6 md:py-10 lg:pt-[60px] pb-3">
        <div className="flex flex-col gap-6 md:gap-12 xl:gap-8">
          ...
          <div className="relative">
            ...
            >
              <CarouselContent className={"2xl:py-4"}>
                {data.map((property) => {
                  return (
                    <CarouselItem
                    ...
                    >
                      ...
                      {type === "properties_by_cities_section" && !with_image ? (
                        <CityCard property={property} />
                      ) : type === "properties_by_cities_section" && with_image ?
                        (
                          <PropertyCityCard property={property} />
