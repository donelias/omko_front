...
const AllListings = () => {
  ...
  const passLocationFilter = useSelector((state) => state?.cacheData?.isLocationBasedHomepageData);
  ...
  const apiConfig = {
    ...
    agents: {
      ...
      length: verifiedUsers.length,
      stateSetter: setVerifiedUsers,
      params: (offset, limit) => ({
        limit: limit?.toString(),
        offset: offset?.toString(),
      }),
  ...
  }, [slug, language, latitude, longitude, radius]);
  ...
  const allListings = [
    ...
    { name: "All Agents", data: verifiedUsers },
  ...
  return (
    <div>
      <NewBreadcrumb
        title={slug === "agents" ? t("allAgents") : t(`all${slug?.charAt(0)?.toUpperCase() + slug?.slice(1)}`)}
                    ...
                    >
                      {item?.data?.length > 0 && item?.data.map((data) => (
                        <CategoryCard
                          ...
                          category={data}
                          passLocationFilter={passLocationFilter}
                        />
                      ))}
                    </div>
                  </>
