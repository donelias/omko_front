...
const ComparePropertyModal = ({
  ...
  const renderDualPrices = (property) => {
      ...
      checkPackageAvailable(PackageTypes.PREMIUM_PROPERTIES).then(
        (isAvailable) => {
          if (isAvailable) {
            handleClose?.();
            router.push(
              `/compare-properties/${currentPropertyId}-vs-${property.id}?lang=${lang}`,
            );
          } else {
                ...
                >
                  <div className="flex w-full flex-col items-center gap-4 sm:flex-row sm:gap-0">
                    ...
                    <div className="flex flex-col items-start w-full px-2">
                      ...
                      {(() => {
                        const { mainPriceString, convertedPriceString } = renderDualPrices(property);
                        return (
                          <div className="flex flex-col">
                            <p className="blackTextColor text-lg font-semibold">{mainPriceString}</p>
