...
const withCacheBustParam = (params = {}) => ({
  ...params,
  _t: Date.now(),
});
...
export const getProjectDetailsApi = async ({
  ...
  const params = { slug_id, get_similar, _t: Date.now() };
  const filteredParams = getFilteredParams(params);
...
export const postProjectApi = async ({
  ...
  if (Array.isArray(plans)) {
    plans.forEach((plans, index) => {
      ...
      if (plans.unit_code !== undefined && plans.unit_code !== null) {
        data.append(`plans[${index}][unit_code]`, plans.unit_code);
      }
      if (plans.price !== undefined && plans.price !== null) {
        data.append(`plans[${index}][price]`, plans.price);
      }
      if (plans.currency !== undefined && plans.currency !== null) {
        data.append(`plans[${index}][currency]`, plans.currency);
      }
      if (plans.total_units !== undefined && plans.total_units !== null) {
        data.append(`plans[${index}][total_units]`, plans.total_units);
      }
      if (plans.available_units !== undefined && plans.available_units !== null) {
        data.append(`plans[${index}][available_units]`, plans.available_units);
      }
      if (plans.unit_status !== undefined && plans.unit_status !== null) {
        data.append(`plans[${index}][unit_status]`, plans.unit_status);
      }
      if (plans.category_id !== undefined && plans.category_id !== null) {
        data.append(`plans[${index}][category_id]`, plans.category_id);
      }
      if (plans.country !== undefined && plans.country !== null) {
        data.append(`plans[${index}][country]`, plans.country);
      }
      if (plans.state !== undefined && plans.state !== null) {
        data.append(`plans[${index}][state]`, plans.state);
      }
      if (plans.city !== undefined && plans.city !== null) {
        data.append(`plans[${index}][city]`, plans.city);
      }
      if (plans.location !== undefined && plans.location !== null) {
        data.append(`plans[${index}][location]`, plans.location);
      }
      if (plans.latitude !== undefined && plans.latitude !== null) {
        data.append(`plans[${index}][latitude]`, plans.latitude);
      }
      if (plans.longitude !== undefined && plans.longitude !== null) {
        data.append(`plans[${index}][longitude]`, plans.longitude);
      }
      if (plans.features !== undefined && plans.features !== null) {
        data.append(`plans[${index}][features]`, JSON.stringify(plans.features));
      }
    });
  }

  // Check if documents is defined and an array before using forEach
  if (Array.isArray(documents)) {
    documents.forEach((image, index) => {
      data.append(`documents[${index}]`, image);
    });
  }
  // Append pre-uploaded document names as strings
  if (Array.isArray(document_names)) {
    document_names.forEach((name, index) => {
      data.append(`document_names[${index}]`, name);
    });
  }
  // Check if gallery_images is defined and an array before using forEach
  if (Array.isArray(gallery_images)) {
  ...
  // Append parameters (Features & Amenities)
  if (Array.isArray(parameters)) {
    parameters.forEach((param, index) => {
      data.append(`parameters[${index}][parameter_id]`, param.parameter_id);
      data.append(`parameters[${index}][value]`, param.value);
    });
  }

  // Append facilities (Outdoor Facilities)
  if (Array.isArray(facilities)) {
    facilities.forEach((facility, index) => {
      data.append(`facilities[${index}][facility_id]`, facility.facility_id);
      data.append(`facilities[${index}][distance]`, facility.distance);
...
export const getAllSimilarPropertiesApi = async ({
  ...
  const params = { property_id, limit, offset, search, _t: Date.now() };
...
// 0. Upload Project Document (individual, before main form)
export const uploadProjectDocumentApi = async (file) => {
  const formData = new FormData();
  formData.append('file', file);
  const res = await api.post(apiEndpoints.UPLOAD_PROJECT_DOCUMENT, formData);
  return res.data;
};
...
export const getHomepagePropertiesOnMapSectionApi = async ({
  ...
  const params = withCacheBustParam(getFilteredParams({
    latitude,
    longitude,
    radius
  }));
...
export const getAdBannerApi = async ({
  ...
  try {
    const res = await api.get(apiEndpoints.GET_AD_BANNERS, {
      params: withCacheBustParam({ page, platform }),
    });
    return res.data;
  } catch (error) {
    // Ad banners are optional content; keep page rendering stable if network blocks this endpoint.
    console.warn("Ad banner request failed, using empty data", {
      page,
      platform,
...
export const getBanksApi = async () => {
  const res = await api.get(apiEndpoints.GET_BANKS);
  return res.data;
};

export const getCooperativesApi = async () => {
  const res = await api.get(apiEndpoints.GET_COOPERATIVES);
  return res.data;
};

export const getFinancialAdvisorsApi = async ({ entity_type = "", entity_id = "" }) => {
  const params = getFilteredParams({ entity_type, entity_id });
  const res = await api.get(apiEndpoints.GET_FINANCIAL_ADVISORS, { params });
  return res.data;
};

export const submitPreQualificationApi = async (formData) => {
  const res = await api.post(apiEndpoints.POST_PRE_QUALIFICATION, formData);
  return res.data;
};

export const getPreQualificationsApi = async () => {
  const res = await api.get(apiEndpoints.GET_PRE_QUALIFICATIONS);
  return res.data;
};

export const getPreQualificationApi = async (id) => {
  const res = await api.get(`${apiEndpoints.GET_PRE_QUALIFICATION}/${id}`);
  return res.data;
};

export const submitAgentPreQualificationApi = async (formData) => {
  const res = await api.post("agent/pre-qualification", formData);
  return res.data;
};
