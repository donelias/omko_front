...
/**
 * Serializes the internal property filter state into clean, readable query
 * params (the inverse of parseFiltersFromQueryString / parseFiltersFromQuery).
 *
 * Emits only the params the SSR parser understands:
 *   q, type, min_price, max_price, posted_since, city, state, country,
 *   promoted, premium, features, sort
 *
 * Values that map to "no filter" are omitted entirely.
 *
 * @param {Object} filters - Internal filter state
 * @param {Object} [options={}] - { sortBy: 'newest'|'most_viewed'|'most_liked'|'' }
 * @returns {Object} Query params object for URLSearchParams
 */
export const buildFiltersQueryParams = (filters = {}, options = {}) => {
  ...
  if (propType === 'sell' || propType === 'buy') params.type = 'sell';
  else if (propType === 'rent') params.type = 'rent';
  ...
  const ps = filters.posted_since;
  if (ps && ps !== '' && String(ps).toLowerCase() !== 'anytime') {
    const psLabel = isNaN(Number(ps)) ? ps : getPostedSinceString(ps);
    if (psLabel) params.posted_since = psLabel;
  }
  ...
  if (filters.promoted === true || filters.promoted === '1' || filters.promoted === 1) params.promoted = '1';
  if (filters.is_premium === true || filters.is_premium === '1' || filters.is_premium === 1) params.premium = '1';

  const amenities = Array.isArray(filters.amenities) ? filters.amenities : [];
  if (amenities.length > 0) {
    params.features = amenities
      .map((a) => parseInt(typeof a === 'object' && a !== null ? a.id : a, 10))
      .filter((id) => id && !isNaN(id))
      .join(',');
  }

  const mostViewed = filters.most_viewed === '1' || filters.most_viewed === true;
  const mostLiked = filters.most_liked === '1' || filters.most_liked === true;
  if (sortBy === 'most_viewed' || mostViewed) params.sort = 'most_viewed';
  else if (sortBy === 'most_liked' || mostLiked) params.sort = 'most_liked';

  return params;
};

/**
 * Parses the clean readable query params back into internal property filter
 * state. Returns null when no clean filter param is present so callers can
 * fall back to their context defaults (legacy base64, locked filters, etc.).
 *
 * Mirrors the clean-params branch of serverFetch.parseFiltersFromQuery so the
 * client reads URLs exactly as the SSR snapshot produced them (and vice-versa).
 *
 * @param {Object} query - Router query object
 * @param {Object} [options={}] - { isCityPage, citySlug, isCategoryPage, categorySlug }
 * @returns {Object|null} Internal filter state or null
 */
export const parseFiltersFromQueryString = (query = {}, options = {}) => {
  const { isCityPage = false, citySlug = '', isCategoryPage = false, categorySlug = '' } = options;
  const q = query || {};
  const val = (key) => (Array.isArray(q[key]) ? q[key][0] : q[key]);

  const cleanKeys = ['q', 'type', 'min_price', 'max_price', 'posted_since', 'city', 'state', 'country', 'promoted', 'premium', 'features', 'sort'];
  const hasCleanParams = cleanKeys.some((key) => {
    const v = val(key);
    return v !== undefined && v !== '';
  });
  if (!hasCleanParams) return null;
  ...
  const searchTerm = val("q");
  if (searchTerm) filters.keywords = searchTerm;

  const rawType = String(val("type") || "").toLowerCase();
  if (rawType === "sell" || rawType === "buy") filters.property_type = "Sell";
  else if (rawType === "rent") filters.property_type = "Rent";

  if (val("min_price")) filters.min_price = val("min_price");
  if (val("max_price")) filters.max_price = val("max_price");
  if (val("posted_since")) filters.posted_since = val("posted_since");
  if (val("city")) filters.city = isCityPage ? (citySlug || val("city")) : val("city");
  if (val("state")) filters.state = isCityPage ? "" : val("state");
  if (val("country")) filters.country = isCityPage ? "" : val("country");
  if (val("promoted") === "1") filters.promoted = true;
  if (val("premium") === "1") filters.is_premium = true;
  ...
  const sort = String(val("sort") || "").toLowerCase();
...
export const getCurrencyCode = () => {
  const country = String(process.env.NEXT_PUBLIC_DEFAULT_COUNTRY || "DO").toUpperCase();
  return COUNTRY_CURRENCY_MAP[country] || "DOP";
};

/**
 * Builds a schema.org RealEstateListing JSON-LD object from a property payload.
 * Only truthy fields are included so the markup stays clean for crawlers.
 *
 * @param {Object} property - Property entity (data.data[0] of get_property)
 * @param {Object} [options={}] - { url, currency }
 * @returns {Object|null} JSON-LD object or null when there is no property slug
 */
export const buildPropertySchema = (property = {}, options = {}) => {
  const { url = "", currency = getCurrencyCode() } = options;
  if (!property?.slug_id && !property?.title) return null;

  const title = property.translated_title || property.title;
  const datePosted = property.created_at || property.post_date || property.created_date || null;
  ...
  const images = [
    ...
    ...(Array.isArray(property.gallery)
      ? property.gallery.map((g) => g?.image_url || g?.image).filter(Boolean)
      : []),
  ].filter(Boolean);
  ...
  if (datePosted) schema.datePosted = safeDate(datePosted);
 ...
 * @returns {Object|null} JSON-LD object or null when the list is empty
 */
