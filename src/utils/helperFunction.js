...
export const formatExpiryDateLabel = (expiryDate, translate = t) => {
  ...
  if (diffDays <= 0) return translate("expired");
...
export const getCurrentLocationData = async () => {
  ...
  const locationData = {
    ...
    state,
    country,
    radius: isDemo ? (Number(webSettings?.min_radius_range) + 10) : Number(webSettings?.min_radius_range) ?? 1,
  };
...
export const generateBase64FilterUrl = (filters, options = {}) => {
  ...
  // Build and clean parameters (amenities)
  const rawParameters = Array.isArray(filters?.amenities) ? filters.amenities : [];
  const cleanParameters = rawParameters.filter(Boolean);

  // Build and clean nearby_places
  const rawNearbyPlaces = Array.isArray(filters?.nearbyPlaces) ? filters.nearbyPlaces : [];
  const cleanNearbyPlaces = rawNearbyPlaces.filter(Boolean);
