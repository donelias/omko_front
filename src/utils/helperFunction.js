...
export const showSwal = ({
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      if (isPremiumCheck)
        router.push(`/subscription-plan?lang=${router?.query?.lang}`);
...
export const handlePackageCheck = async (
  ...
  if ([PackageTypes.PROPERTY_LIST, PackageTypes.PROJECT_LIST].includes(type)) {
    ...
    if (!isProfileComplete) {
      return showSwal({
        ...
        onConfirm: () => router.push(`/user/profile?lang=${locale}`),
        t
      });
    }
    ...
    if (needToVerify && verificationStatus !== "success") {
      const statusMessages = {
        ...
        failed: {
          ...
          onConfirm: () => router.push(`/user/dashboard?lang=${locale}`),
          t
        },
        default: {
          icon: "error",
          ...
          onConfirm: () => router.push(`/user/verification-form?lang=${locale}`),
          t
        },
      };
  ...
  const successActions = {
    [PackageTypes.PROPERTY_LIST]: () =>
      router.push(`/user/add-property?lang=${locale}`),
    [PackageTypes.PROJECT_LIST]: () =>
      router.push(`/user/add-project?lang=${locale}`),
    [PackageTypes.PROPERTY_FEATURE]: async () => {
      try {
        ...
        router.push(`/user/advertisement?tab=property&lang=${locale}`);
      } catch (err) {
    ...
    [PackageTypes.PROJECT_FEATURE]: async () => {
      try {
        ...
        toast.success(t(res.message));
        router.push(`/user/advertisement?tab=project&lang=${locale}`);
      } catch (err) {
        toast.error(t(err.message));
      }
    },
...
export const getProjectFilters = (lang = "") => {
  const locationData = store.getState()?.location || {};
  const location = {
    latitude: locationData?.latitude || "",
    longitude: locationData?.longitude || "",
  ...
  return `?latitude=${location.latitude}&longitude=${location.longitude}&range=${location.range}&lang=${lang}`;
}
