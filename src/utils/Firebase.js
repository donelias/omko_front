...
const FirebaseData = () => {
  ...
  const fetchToken = async (setTokenFound, setFcmToken) => {
    try {
      ...
      // Check if VAPID_KEY is valid (not placeholder)
      // Check for placeholder strings or empty values
      if (!vapidKey || vapidKey.startsWith("xxx") || vapidKey.length < 20) {
        console.warn("VAPID_KEY not configured. Push notifications will not work.");
        setTokenFound?.(false);
        setFcmToken?.(null);
        return;
      }
      ...
      try {
        ...
        if (tokenErr.message.includes("applicationServerKey")) {
          console.error("Invalid VAPID_KEY. Please check Firebase Cloud Messaging settings.");
        }
        setTokenFound?.(false);
        setFcmToken?.(null);
      }
