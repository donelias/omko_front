...
import { generateSlug, placeholderImage, translate } from "@/utils";
import { compressImage, compressImages, IMAGE_COMPRESSION_CONFIG } from "@/utils/imageCompression";
import { GetAllCategorieApi, GetFacilitiesApi, PostProperty } from "@/store/actions/campaign";
...
export default function AddPropertyTabs() {
    ...
    const DistanceSymbol = systemSettingsData && systemSettingsData?.distance_option;
    ...
    const handleNextTab3 = (e) => {
        ...
        const requiredParams = categoryParameters.filter(param => param.is_required === 1);

        // Collect names of missing required parameters
        const missingParams = requiredParams.filter(param => !tab2[param.id] || tab2[param.id] === "")
    ...
    const handlePostproperty = async (e) => {
        ...
        try {
            if (!tab1.category) {
                ...
                const threeDImage = tab5._3DImages && tab5._3DImages.length > 0 ? tab5._3DImages[0] : null;
                const galleryImages = tab5.galleryImages && tab5.galleryImages.length > 0 ? tab5.galleryImages : [];
                const titleImage = tab5.titleImage && tab5.titleImage.length > 0 ? tab5.titleImage[0] : null;
                const ogImages = tab6.ogImages && tab6.ogImages.length > 0 ? tab6.ogImages[0] : null;

                // Compress images before sending with specific configurations (1024 x 584)
                let compressedTitleImage = titleImage;
                let compressedGalleryImages = galleryImages;
                let compressedThreeDImage = threeDImage;
                let compressedOgImage = ogImages;

                try {
                    // Compress title image with its specific config
                    if (compressedTitleImage) {
                        const config = IMAGE_COMPRESSION_CONFIG.titleImage;
                        compressedTitleImage = await compressImage(
                    ...
                    // Compress gallery images with their specific config
                    if (compressedGalleryImages && compressedGalleryImages.length > 0) {
                        const config = IMAGE_COMPRESSION_CONFIG.galleryImages;
                        compressedGalleryImages = await compressImages(
                    ...
                    // Compress 3D image with its specific config
                    if (compressedThreeDImage) {
                        const config = IMAGE_COMPRESSION_CONFIG.threeDImage;
                        compressedThreeDImage = await compressImage(
                            compressedThreeDImage,
                            config.maxSizeMB,
                            config.width,
                            config.height,
                            config.quality
                    ...
                    // Compress OG image with its specific config
                    if (compressedOgImage) {
                        const config = IMAGE_COMPRESSION_CONFIG.ogImage;
                        compressedOgImage = await compressImage(
                            compressedOgImage,
                            config.maxSizeMB,
                            config.width,
                            config.height,
                ...
                const formattedSlug = tab1.slug ? generateSlug(tab1.slug) : generateSlug(tab1.title);
                PostProperty({
                    // userid: userId,
                    title: tab1.title,
                    ...
                    onError: (error) => {
                        const errorMessage = typeof error === 'string' ? error : error?.message || 'Error al crear la propiedad';
                        ...
                        <div id="add_prop_form">
                            ...
                            {tab1.propertyType !== "1" ? (
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-sm-12 col-md-6 col-lg-4">
                                        <div className="add_prop_fields">
                                            <span className="is_require">{translate("RentDuration")}</span>
                                            <select
                    ...
                    {categoryParameters && categoryParameters.length > 0 ? (
                        categoryParameters.map((ele, index) => (
                            <div className="col-sm-12 col-md-6 col-lg-3" key={index}>
                                <div className="add_prop_fields">
                                    <span className={ele?.is_required === 1 ? "is_require" : ""}>{ele?.name}</span>
                    ...
                    {getFacilities.length > 0
                        ? getFacilities.map((ele, index) => (
                            <div className="col-sm-12 col-md-6 col-lg-3" key={index}>
                                <div className="add_prop_fields">
                                    <span>{ele?.name}</span>
                                    <input value={tab3[ele?.id] || ""} type="number" placeholder={`00 ${DistanceSymbol}`} className="prop_number_input" id={`prop_title_input_${ele?.id}`} onChange={(e) => handleTab3InputChange(ele?.id, e.target.value)} />
                                </div>
                            </div>
                        ))
                        : null}
