...
const getDynamicRouteConfigs = () => [
  ...
  },
  {
    ...
    apiEndpoint: `${getApiBase()}get_categories`,
    // Only categories that actually have listings should be indexed
    extraParams: { has_property: true },
    pathExtractor: (item) => item?.slug_id,
    priority: 0.8,
    changefreq: 'weekly',
  },
...
/**
 * Fetches distinct city slugs from the property list.
 *
 * The cities endpoint (get-cities-data) only exposes display names (City/Count)
 * with no stable slug, so city landing pages are derived directly from the
 * `city` field of each property — the exact value used by CityCard /
 * PropertyCityCard links. This automatically covers whichever country data is
 * live (e.g. RD: Santo Domingo, Santiago, Moca, Punta Cana).
 *
...
const fetchCityRoutes = async () => {
  ...
  const fetchPage = async () => {
    // Try without `filters` first (the demo API returns 0 results for an empty
    // filters object, which would make city aggregation useless). Fall back to
    // the encoded empty filters if the bare call fails (stricter APIs may require it).
    try {
      return await axios.get(`${getApiBase()}get-property-list`, {
        params,
        timeout: 10000,
        headers,
      });
      ...
      try {
        return await axios.get(`${getApiBase()}get-property-list`, {
          params: { ...params, filters: buildPropertyFilters() },
          timeout: 10000,
          headers,
        });
      } catch {
  ...
  while (true) {
    try {
      ...
      data.forEach((item) => {
        const city = (item?.city || item?.City || '').toString().trim();
      ...
      console.error(`Error fetching cities from property list at offset ${offset}:`, err.message);
      break;
    }
  }
...
const generateSitemapXml = async ({
  ...
  // City landing pages (distinct city values from live property data)
  const cityRoutes = await fetchCityRoutes();
  cityRoutes.forEach(({ path, priority, changefreq, lastmod }) => {
    const normalizedPath = path.replace(/^\/+/, '');
    sitemapEntries.push(
      generateUrlEntry(normalizedPath, priority, changefreq, lastmod, languages, defaultLangCode)
    );
