...
class ApiController extends Controller
{
    ...
    {
        $sliderData = Slider::select('id','type', 'image', 'category_id', 'propertys_id','show_property_details','link')->with('category:id,category','property:id,title,title_image,price,propery_type as property_type')->orderBy('id', 'desc')->get()->map(function($slider){
    ...
    public function get_categories(Request $request)
    {
        ...
        $categories = Category::select('id', 'category', 'names', 'image', 'parameter_types', 'meta_title', 'meta_description', 'meta_keywords', 'slug_id')->where('status', '1');
        ...
        // Mapear resultados para asegurar que retornen el nombre en el idioma correcto
        $data = [];
        foreach ($result as $category) {
            // Convertir a array (usa el método toArray() que aplica traducción)
            $categoryArray = $category->toArray();
            $categoryArray['meta_image'] = $category->image;
            
            // Agregar parameter_types
            $parameterData = parameterTypesByCategory($category->id);
            ...
            $categoryArray['parameter_types'] = $parameterData;
    ...
    public function paypal(Request $request)
    {
        ...
        if (!$validator->fails()) {
            $current_user = Auth::user()->id;
            $paypal = new Paypal();
            $webURL = system_setting('paypal_web_url');
    ...
    public function app_payment_status(Request $request)
    {
        ...
        // Get Web URL
        $webURL = system_setting('paypal_web_url') ?? null;
    ...
    public function send_message(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'audio' => 'nullable|mimes:audio/mpeg|max:5024'
        ]);
        if ($validator->fails()) {
            return response()->json([
                'error' => true,
    ...
    public function get_facilities(Request $request)
    {
        ...
        // Mapear los resultados para asegurar que retornen el nombre en el idioma correcto
        $data = $result->map(function ($facility) {
            // El accessor getNameAttribute() automáticamente retorna el nombre localizado
            return [
                'id' => $facility->id,
                'name' => $facility->name, // Automáticamente traducido por el accessor
    ...
    public function get_projects(Request $request)
    {
        ...
        if (isset($request->search) && !empty($request->search)) {
            ...
            $project = $project->where(function ($query) use ($search) {
                $query->where('title', 'LIKE', "%$search%")->orwhereHas('category', function ($query1) use ($search) {
                    $query1->where('category', 'LIKE', "%$search%");
                });
            });
        }
        ...
        $result = $project->skip($offset)->take($limit)->orderBy('id','DESC')->get();
    ...
    public function homepageData()
    {
        try {
            //Faq Data
            $faqsData = Faq::select('id','question','answer','contents')->where('status',1)->orderBy('id','DESC')->limit(5)->get();
            ...
            //Slider Data
            $slidersData = Slider::select('id','type', 'image', 'category_id', 'propertys_id','show_property_details','link')->with(['category:id,slug_id,category','property' => function($query){
                $query->whereIn('propery_type',array(0,1))->select('id','slug_id','title','title_image','price','propery_type as property_type');
    ...
    public function getFaqData(Request $request)
    {
        try {
            ...
            $faqsData = $faqsQuery->clone()->select('id','question','answer','contents')->orderBy('id','DESC')->skip($offset)->take($limit)->get();
            $response = array(
                'error' => false,
                'total' => $totalData ?? 0,
                'data' => $faqsData,
    ...
    public function getPropertyList(Request $request){
        try {
            ...
            $property_type = $request->property_type;  //0 : Sell 1:Rent
            if (isset($property_type) && (!empty($property_type) || $property_type == 0)) {
                $propertyQuery = $propertyQuery->where('propery_type', $property_type);
            }

            // If Category Id is Passed
            if ($request->has('category_id') && !empty($request->category_id)) {
                $propertyQuery = $propertyQuery->where('category_id', $request->category_id);
            }
            ...
            // If Country is passed
            if ($request->has('country') && !empty($request->country)) {
                $propertyQuery = $propertyQuery->where('country', $request->country);
            }

            // If State is passed
            if ($request->has('state') && !empty($request->state)) {
                $propertyQuery = $propertyQuery->where('state', $request->state);
            }

            // If City is passed
            if ($request->has('city') && !empty($request->city)) {
                $propertyQuery = $propertyQuery->where('city', $request->city);
            }
            ...
            if ($request->has('posted_since')) {
                ...
                if ($posted_since == 0) {
                    $startDateOfWeek = Carbon::now()->subWeek()->startOfWeek();
                    $endDateOfWeek = Carbon::now()->subWeek()->endOfWeek();
                    $propertyQuery = $propertyQuery->whereBetween('created_at', [$startDateOfWeek, $endDateOfWeek]);
                }
                // 1 - Yesterday
                if ($posted_since == 1) {
                    $yesterdayDate = Carbon::yesterday();
            ...
            if ($request->has('search') && !empty($request->search)) {
                $search = $request->search;
                $propertyQuery = $propertyQuery->where(function ($query) use ($search) {
                    $query->where('title', 'LIKE', "%$search%")
                        ->orWhere('address', 'LIKE', "%$search%")
                        ->orWhereHas('category', function ($query1) use ($search) {
                            $query1->where('category', 'LIKE', "%$search%");
            ...
            if ($request->has('promoted') && $request->promoted == 1) {
                $propertyQuery = $propertyQuery->whereHas('advertisement',function($query){
                    $query->where(['status' => 'Approved', 'is_enable' => 1]);
                });
            }
