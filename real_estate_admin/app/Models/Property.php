...
class Property extends Model
{
    ...
    public function getIsPromotedAttribute() {
        ...
        return $this->whereHas('advertisement',function($query) use($id){
            $query->where(['property_id' => $id, 'status' => 'Approved', 'is_enable' => 1]);
        })->count() ? true : false;
    }
    ...
    public function getHomePromotedAttribute() {
        $id = $this->id;
        return $this->whereHas('advertisement',function($query) use($id){
            $query->where(['property_id' => $id,'type' => 'HomeScreen', 'status' => 'Approved', 'is_enable' => 1]);
        })->count() ? true : false;
    }
    ...
    public function getListPromotedAttribute() {
        $id = $this->id;
        return $this->whereHas('advertisement',function($query) use($id){
            $query->where(['property_id' => $id,'type' => 'ProductListing', 'status' => 'Approved', 'is_enable' => 1]);
        })->count() ? true : false;
    }
