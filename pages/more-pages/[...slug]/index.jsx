...
const fetchDataFromSeo = async (pageName) => {
    try {
        const response = await axios.get(
            `${process.env.NEXT_PUBLIC_API_URL}${process.env.NEXT_PUBLIC_END_POINT}${GET_SEO_SETTINGS}?page=custom-page/${pageName}`
        );
...
if (process.env.NEXT_PUBLIC_SEO === "true") {
    serverSidePropsFunction = async (context) => {
        ...
        const seoData = await fetchDataFromSeo(query?.slug[0]);
        return {
            props: {
                seoData,
                pageName,
